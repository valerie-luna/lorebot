...
public sealed class SimpleDiscordDebugDiceFormatter : DiceFormatter<DiscordInteractionResponseBuilder>
{
    ...
    protected override DiscordInteractionResponseBuilder FormatSingle(Expression fullExpr, Roll roll, string? reason)
    {
        ...
        final = $"{final}\n**Seed:** {roll.Seed}"
            + $"\n**Roll Type:** {new RangeDeterminatorVisitor().Visit(roll.Expression)}";
        if (roll.Desugared is not null)
        {
            final += $" ({new RangeDeterminatorVisitor().Visit(roll.Desugared)})";
        }
        return new DiscordInteractionResponseBuilder()
            .WithContent(final);
    }
    ...
    protected override DiscordInteractionResponseBuilder FormatMultiple(Expression fullExpr, IEnumerable<Roll> rolls, string? reason)
    {
        ...
        foreach (var (Expression, Desugared, Result, Seed) in rolls)
        {
            ...
            sb.Append($"**Seed:** `{Seed}`"
                + $"\n**Roll Type:** `{new RangeDeterminatorVisitor().Visit(Expression)}`");
            if (Desugared is not null)
            {
                sb.AppendLine($" (`{new RangeDeterminatorVisitor().Visit(Desugared)}`)");
            }
            else
            {
                sb.AppendLine();
